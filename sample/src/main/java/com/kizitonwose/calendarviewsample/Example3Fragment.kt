    ...
    inner class Example3EventsViewHolder(private val binding: Example3EventItemViewBinding) :
        ...
        init {
            itemView.setOnClickListener {
                ...
                Log.i("Example3Fragment", "Hello")
               //  Toast.makeText(getActivity(Exam ),"helo",Toast.LENGTH_SHORT ).show()
            ...
            init {
                view.setOnClickListener {
                    if (day.owner == DayOwner.THIS_MONTH) {
                        selectDate(day.date)
                        // toast
                       // Toast.makeText(context, day.date.toString(), Toast.LENGTH_SHORT).show()

                        // array string
                        val appThemeList = resources.getStringArray(R.array.vachans)
                        ...
                        builder.setTitle("Vachans")
                        //set message for alert dialog
                        builder.setMessage(appThemeList[101])
                        builder.setIcon(android.R.drawable.ic_input_add)
                        builder.setPositiveButton("Share"){dialogInterface, which ->
                            Toast.makeText(context,"clicked yes",Toast.LENGTH_LONG).show()
                            val shareIntent = Intent()
                            shareIntent.action = Intent.ACTION_SEND
                            shareIntent.putExtra(Intent.EXTRA_TEXT, appThemeList[101] + "\n\n\n\t\t" + "Daily Vachans app"+ "\n Google Play Store link")
